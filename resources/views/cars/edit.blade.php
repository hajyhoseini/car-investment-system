    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <form method="POST" action="{{ route('cars.update', $car) }}" enctype="multipart/form-data" id="carForm" x-data="wizardForm()">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <x-searchable-select
                                    ...
                                    :selected="$car->purchase_priority ?? 'medium'"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <x-searchable-select
                                    ...
                                    :selected="$car->technical_condition"
...
<script>
    function wizardForm() {
        return {
            ...
            formData: {
                title: '{{ old("title", $car->title) }}',
                brand: '{{ old("brand", $car->brand) }}',
                model: '{{ old("model", $car->model) }}',
                year: '{{ old("year", $car->year) }}',
                kilometers: '{{ old("kilometers", $car->kilometers) }}',
                color: '{{ old("color", $car->color) }}',
                purchase_price: '{{ old("purchase_price", $car->purchase_price) }}',
                purchase_date: '{{ old("purchase_date", $car->jalali_purchase_date ?? "") }}',
                inquiry_date: '{{ old("inquiry_date", $car->jalali_inquiry_date ?? "") }}',
                showroom_name: '{{ old("showroom_name", $car->showroom_name) }}',
                transmission: '{{ old("transmission", $car->transmission) }}',
                fuel_type: '{{ old("fuel_type", $car->fuel_type) }}',
                phone_number: '{{ old("phone_number", $car->phone_number) }}',
                document_status: '{{ old("document_status", $car->document_status) }}',
                storage_location: '{{ old("storage_location", $car->storage_location) }}',
                holding_price: '{{ old("holding_price", $car->holding_price) }}',
                purchase_priority: '{{ old("purchase_priority", $car->purchase_priority ?? "medium") }}',
                body_condition: '{{ old("body_condition", $car->body_condition) }}',
                technical_condition: '{{ old("technical_condition", $car->technical_condition) }}',
                owner_type: '{{ old("owner_type", $car->owner_type) }}',
                market_price: '{{ old("market_price", $car->market_price) }}',
                min_price: '{{ old("min_price", $car->min_price) }}',
                customer_type: '{{ old("customer_type", $car->customer_type) }}',
                description: '{{ old("description", $car->description) }}',
                listing_url: '{{ old("listing_url", $car->listing_url) }}'
            },
            errors: {},
