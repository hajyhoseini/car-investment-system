        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                    <form method="GET" action="{{ route('car-sales.index') }}" class="grid grid-cols-1 md:grid-cols-5 gap-4">
                        <!-- جستجو بر اساس خودرو یا خریدار -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">جستجو</label>
                            <input type="text" name="search" value="{{ request('search') }}" 
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" 
                                   placeholder="نام خودرو یا خریدار...">
                        </div>
                        ...
                        <!-- فیلتر بازه تاریخ از -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">از تاریخ فروش</label>
                            <input type="text" name="start_date" id="start_date" value="{{ request('start_date') }}" 
                                   class="jalali-datepicker w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" 
                                   placeholder="مثال: ۱۴۰۲/۰۱/۰۱" autocomplete="off">
                        </div>

                        <!-- فیلتر بازه تاریخ تا -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">تا تاریخ فروش</label>
                ...
                <!-- آمار فروش با در نظر گرفتن فیلترها -->
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                    ...
                    <div class="bg-green-50 p-4 rounded-lg">
                        ...
                        <div class="text-2xl font-bold text-green-600">{{ fa_currency($totalAmount) }}</div>
                    </div>
                    <div class="bg-purple-50 p-4 rounded-lg">
                        <div class="text-sm text-gray-600">کل سود</div>
                        <div class="text-2xl font-bold text-purple-600">{{ fa_currency($totalProfit) }}</div>
                    </div>
                    <div class="bg-yellow-50 p-4 rounded-lg">
                        <div class="text-sm text-gray-600">میانگین سود</div>
                        <div class="text-2xl font-bold text-yellow-600">{{ fa_currency($averageProfit) }}</div>
                    </div>
                </div>
    ...
    <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr class="hover:bg-gray-50 transition">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                    @if($sale->person)
                        <a href="{{ route('people.show', $sale->person) }}" class="text-blue-600 hover:text-blue-900">
                            {{ $sale->person->full_name ?? $sale->buyer_name }}
                        </a>
                    @else
                        <span class="text-gray-500">{{ $sale->buyer_name }}</span>
                    @endif
                </td>
